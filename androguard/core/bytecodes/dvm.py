...
#-----------
	if self.class_data_item != None:
		...
		#str1+=i.get_name()
		#str1+=" "
		str1+=i.get_descriptor()
		self.cls_methd.append(str1)
		...
		if code!=None:
			...
			if n==1:
				...
				for i in get_pack_call(buff):
				    if i not in self.cls_used_cls:
					self.cls_used_cls.append(i) 	    		
	    fields=self.class_data_item.get_fields()
	    str1=""
	    for i in fields:
		str1=""	
		str1+=i.get_access_flags_string()
		str1+=" "
		str1+=i.get_descriptor()
		#str1+=" "
		#str1+=i.get_name()
		self.cls_field_var.append(str1)
...
#---------
	...
	for i in self.class_def:
	    if i.get_name() == name_class:
		str1="\nMethod List: "
		str1+=str(i.cls_methd)
		file_d.write(str1)
		file_d.write("\n")
		class_field=i.get_fields()
		str4="\nFields: "
		for k in class_field:
		    str4+="\n\t"
		    str4+=k.get_access_flags_string()
		    str4+=" "		
		    str4+=k.get_descriptor()
		    str4+=" "
		    str4+=k.get_name()	
#		str4+=str(i.get_fields())
		str4+="\n"
		file_d.write(str4)
		file_d.write("\n")
		str1="Fields2: "+str(i.cls_field_var)  
		file_d.write(str1)
		file_d.write("\n")
		str1="Used Class: "
		str1+=str(i.cls_used_cls)
		str1+="\n"
		file_d.write(str1)
		for j in i.get_methods():
		    str1= "\n\tFunc: %s %s %s %d "%(name_class,j.get_name(),j.get_descriptor(),j.get_length())
		    str1=str1+"\n" 	
		    file_d.write(str1)
		    str2="\t\t"		
		    str2+=self.get_each_method_api_sig(j)
		    str2+="\n"	
		    file_d.write(str2)
		    str3="method_sig: %s\n" %(j.sig)
		    file_d.write(str3)
#---------!
#---------
    def get_each_method_sig(self,method_name):
	code = method_name.get_code()
	buff=""
	if code != None :
            bc = code.get_bc()
...
#---------!
#---------
    def get_each_method(self,name_class):
	for i in self.class_def:
	    if i.get_name() == name_class:
		for j in i.get_methods():
		    print ("\t%s %s %s %d ")%(name_class,j.get_name(),j.get_descriptor(),j.get_length())	
		    self.get_each_method_sig(j)   	
#---------!
#---------
    def get_method_name(self,name_class):
	for i in self.class_def:
	    if i.get_name() == name_class:
		for j in i.get_methods():
		    print ("\t%s %s %s %d ")%(j.get_class_name(),j.get_name(),j.get_descriptor(),j.get_length())
#---------!      		
    def get_names(self) :
#---------
...
#        print i, hex(h[ i ])
	...
	#print self.classes.class_def
#	for i in self.classes_names:
#		print self.get_class(i)	
